  ...
  <header class="pt-1 px-4 w-100 navbar-expand-xxl bg-dark shadows fixed-top">
    ...
    <div
      ...
      <form class="d-flex mt-3 mt-lg-0 " role="search">
        ...
        <button class="btn btn-outline-light me-2" type="submit"><svg xmlns="http://www.w3.org/2000/svg" width="16"
  ...
  <div class="background-profile">
    <div class="bg-top text-center">
      ...
      if ($User) {
        ...
        if (isset($User['cover_image']) && file_exists($User['cover_image'])) {
          $image = $image_class->get_thumb_cover($User['cover_image']);
        }
        ?>
        ...
        if (isset($User['first_name']) && isset($User['last_name'])) {
          echo '<div class="Name-profile">' . $User['first_name'] . ' ' . $User['last_name'] . '</div>';
        } else {
          echo '<div class="Name-profile">User Name Not Found</div>';
        }
        ?>
      <?php } else {
      ...
      if ($row['userid'] == $user_id )
      {
        ...
        <div class='change-profile '>
          <button class='btn btn-outline-info me-3'><a href='./change_profile_image.php?change=profile'
              class='nav-link mb-1  text-center rounded-sm'>แก้ไขโปรไฟล์</a></button>
        </div>
        <div class='change-profile'>
          <button class='btn btn-outline-info'><a href='./change_profile_image.php?change=cover'
              class='nav-link mb-1  text-center rounded-sm'>เปลี่ยนพื้นหลังโปรไฟล์</a></button>
        </div>
      </div>";
      }
